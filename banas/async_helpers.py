...
In production (BASE_URL + QSTASH_TOKEN set): publishes task to QStash,
which calls the registered HTTP endpoint with retries.
...
def async_task(endpoint_path):
    """
    Decorator that publishes a task to QStash (production) or runs it
    in a background thread (local dev).
    ...
    Usage:
        @async_task("/api/tasks/send-email/")
        ...
        def wrapper(*args, **kwargs):
            ...
            if qstash_client and base_url:
                ...
                try:
                    ...
                    logger.info(f"Published async task '{func.__name__}' to QStash → {full_url}")
                except Exception as e:
                    logger.error(f"QStash publish failed for '{func.__name__}': {e}. Falling back to thread.", exc_info=True)
                    # Fallback to thread if QStash publish fails
