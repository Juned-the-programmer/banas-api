...
def daily_entry_count(request):
    if request.method == "GET":
        ...
        return JsonResponse(
            {"today_customer_count": today_customer_count, "today_coolers_total": today_coolers_total},
...
def view_delete_daily_entry(request, pk):
    ...
    if request.method == "DELETE":
        if DailyEntry.objects.filter(id=pk).filter(date_added__gte=first_date, date_added__lte=last_date):
            dailyEntry = DailyEntry.objects.get(pk=pk)
            dailyEntry.delete()
            return JsonResponse({"message": "DailyEntry deleted successfully"}, status=status.HTTP_200_OK)
    ...
    if request.method == "POST":
        ...
        if serializer.is_valid():
            ...
            for data_item in serializer.validated_data:
                ...
                daily_entry_data = DailyEntry(customer=customer_id, cooler=cooler, addedby=addedby, date_added=timezone.now())
        ...
        if serializers.is_valid():
            ...
            for data_item in serializers.initial_data:
                ...
                try:
                    ...
                    return JsonResponse(
                        {"error": f"Error with customer ID {customer_id}: {str(e)}"}, status=status.HTTP_400_BAD_REQUEST
                    )
    ...
    if request.method == "POST":
        ...
        if qr_code_pin == int(request.POST["pin"]):
            pending_daily_entry_customer = pending_daily_entry(
                customer=Customer.objects.get(id=pk), coolers=int(request.POST["coolers"])
            )
...
def historical_data_retriever(request):
    if request.method == "GET":
        ...
        return JsonResponse({f"historical_{params}": old_data.data}, status=status.HTTP_200_OK, safe=False)
