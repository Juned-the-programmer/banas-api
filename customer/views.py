...
def list_customer_by_route(request, pk):
    if request.method == "GET":
        ...
        return JsonResponse(customer_route_serializer.data, status=status.HTTP_200_OK, safe=False)
...
def due_customer(request, pk):
    if request.method == "GET":
        ...
        return JsonResponse(
            {"customer_name": customer.customer_name.first_name + " " + customer.customer_name.last_name, "due": customer_due},
...
def customer_detail(request, pk):
    ...
    if request.method == "GET":
        ...
        customer_daily_entry = (
            customer.customer_daily_entry.filter(customer=customer.id).only("id", "customer").order_by("-date_added")
        )
